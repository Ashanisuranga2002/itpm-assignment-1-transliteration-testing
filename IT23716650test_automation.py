...
THIRD_INPUT_OVERRIDE = "malli, durata yanna epa"
...
def _find_column_index(header_values: List, requested_name: Optional[str], candidates: List[str]) -> Optional[int]:
    indexed = []
...
def _safe_click(locator, timeout_ms: int = 2000) -> bool:
    try:
    ...
    try:
        ...
        try:
            locator.click(timeout=max(500, int(timeout_ms)), force=True)
            return True
        except Exception:
            return False
...
def _wait_for_output_change(page, is_chat: bool, output_locator, prev_output: str, timeout_ms: int, poll_ms: int = 250) -> str:
    try:
        page.wait_for_function(
            ...
            output_locator,
            prev_output or "",
            timeout=max(500, int(timeout_ms)),
        )
    except Exception:
        pass
    ...
    while time.time() < deadline:
        ...
        if current and (not prev_output or current != prev_output):
            return current
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            case_number += 1
            if case_number == 3 and THIRD_INPUT_OVERRIDE:
                singlish_input = THIRD_INPUT_OVERRIDE
                print(f"Overriding input for case 3: {singlish_input}")
            ...
            try:
                ...
                if action_locator:
                    clicked = _safe_click(action_locator, timeout_ms=3000)
                    if not clicked:
                        try:
                ...
                extra_wait_ms = max(1, int(args.retries)) * max(0, int(args.retry_wait_ms))
                wait_total_ms = max(int(args.timeout_ms), int(args.wait_ms) + extra_wait_ms)
